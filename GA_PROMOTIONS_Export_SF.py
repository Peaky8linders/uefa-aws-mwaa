...
USE UEFA_DEV_IDP;
USE SCHEMA GA_PILOT;
COPY INTO UEFA_DEV_IDP.GA_PILOT.GA_PROMOTIONS_GA_SESSIONS_MWAA
from @GA_INGESTION/promotions
ON_ERROR='CONTINUE' 
...
GA_EXPORT_CMD = """
EXPORT DATA OPTIONS (
    uri='gs://ict-ga-bucket/promotions/@file*.csv.gz',
    format='CSV',
    compression='GZIP',
    overwrite=TRUE
) AS
...
CROSS JOIN UNNEST(hits.promotion) as promotions
"""
...
DAG_ID = "BIGQUERY_GA_PROMOTIONS_EXPORT_SNOWFLAKE"

def ga_dq_checks():
    table='PROMOTIONS'
    query=GA_QUERY_PROMOTIONS
...
with DAG(
    ...
    t_export_ga_data_bigquery = BigQueryInsertJobOperator(
        ...
        configuration={
            "query": {
                "query": GA_EXPORT_CMD.replace('@file','PROMOTIONS') + GA_QUERY_PROMOTIONS.replace('@date',datetime.strftime(datetime.now() - timedelta(1), '%Y%m%d')),
                "useLegacySql": False,
                "priority": "BATCH",
            }
        },
